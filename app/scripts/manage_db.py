...
class DatabaseManager:
    ...
    def __init__(self, db_path: str, backup_dir: str, alembic_ini: str):
        ...
        loggers['operations'].info(_("DatabaseManager initialized",
                     db_path=str(db_path),
    ...
    def ensure_directories(self):
        ...
        try:
            ...
            loggers['operations'].info(_("Directories verified",
                         backup_dir=str(self.backup_dir),
                         db_dir=str(self.db_path.parent)))
        except Exception as e:
            loggers['errors'].error(_("Failed to create directories",
                          error=str(e),
    ...
    def backup_database(self):
        ...
        if not self.db_path.exists():
            loggers['backups'].info(_("No database file exists, skipping backup",
        ...
        try:
            loggers['backups'].info(_("Creating database backup",
            ...
            loggers['backups'].info(_("Backup created successfully",
            ...
            loggers['errors'].error(_("Backup failed",
                          error=str(e),
                          traceback=traceback.format_exc()))
            raise
    ...
    def _cleanup_old_backups(self, keep_count: int = 5):
        ...
        try:
            ...
            for old_backup in backups[keep_count:]:
                loggers['backups'].info(_("Removing old backup",
                             backup_path=str(old_backup)))
                old_backup.unlink()
        except Exception as e:
            loggers['errors'].warning(_("Failed to cleanup old backups",
                            error=str(e),
                            traceback=traceback.format_exc()))
    ...
    def get_current_revision(self) -> str:
        ...
        if not self.db_path.exists():
            loggers['operations'].info(_("No database file exists",
                         db_path=str(self.db_path)))
            return None
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                loggers['operations'].info(_("Current database revision retrieved",
                             revision=revision))
                return revision
            ...
            loggers['errors'].error(_("Error getting current revision",
                          error=str(e),
                          traceback=traceback.format_exc()))
            raise
    ...
    def get_head_revision(self) -> str:
        ...
        try:
            ...
            loggers['operations'].info(_("Head revision retrieved",
                         revision=head))
            return head
        except Exception as e:
            loggers['errors'].error(_("Error getting head revision",
                          error=str(e),
                          traceback=traceback.format_exc()))
            raise
    ...
    def needs_upgrade(self) -> bool:
        ...
        try:
            ...
            loggers['operations'].info(_("Checked upgrade status",
                         current_revision=current,
                         head_revision=head,
                         needs_upgrade=needs_upgrade))
            return needs_upgrade
        except Exception as e:
            loggers['errors'].error(_("Error checking upgrade status",
                          error=str(e),
                          traceback=traceback.format_exc()))
            raise
    ...
    def init_database(self):
        ...
        try:
            ...
            loggers['operations'].info(_("Creating new database",
                         db_path=str(self.db_path)))
            ...
            loggers['errors'].error(_("Database initialization failed",
                          error=str(e),
                          traceback=traceback.format_exc()))
            if self.db_path.exists():
                self.db_path.unlink()
    ...
    def run_migrations(self):
        ...
        try:
            ...
            if current is None:
                ...
                loggers['migrations'].info(_("Running migrations",
            ...
            if new_current != head:
                raise Exception(f"Migration failed: current revision {new_current} does not match head {head}")
            
            loggers['migrations'].info(_("Migrations completed successfully",
                         current_revision=new_current))
            return True
        except Exception as e:
            loggers['errors'].error(_("Migration failed",
                          error=str(e),
                          traceback=traceback.format_exc()))
            return False
    ...
    def validate_database(self) -> bool:
        ...
        """
        if not self.db_path.exists():
            loggers['operations'].info(_("Database file does not exist",
                         db_path=str(self.db_path)))
            return False
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if missing_tables:
                    loggers['operations'].warning(_("Missing required tables",
                ...
                required_columns = {
                    'id', 'content', 'qr_type', 'redirect_url', 'created_at',
                    'scan_count', 'last_scan_at', 'fill_color', 'back_color',
                    'size', 'border'
                }
                missing_columns = required_columns - columns
                
                if missing_columns:
                    loggers['operations'].warning(_("Missing required columns in qr_codes table",
            ...
            loggers['operations'].warning(_("Invalid SQLite database",
                             error=str(e)))
            return False
        except Exception as e:
            loggers['errors'].error(_("Error validating database",
                          error=str(e),
                          traceback=traceback.format_exc()))
            return False
...
def main():
    ...
    try:
        ...
        loggers['errors'].error(_("Script execution failed",
                      error=str(e),
                      traceback=traceback.format_exc()))
        sys.exit(1)
