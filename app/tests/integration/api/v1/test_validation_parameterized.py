    ...
    if expected_status == 200 and expected_attrs:
        ...
        for key, value in expected_attrs.items():
            ...
            if key == "redirect_url" and value is not None:
                assert data[key].rstrip("/") == value.rstrip(
                    "/"
                ), f"Expected {value} for {key}, got {data[key]}"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            200,
            lambda data, payload: data["redirect_url"].rstrip("/")
            == payload["redirect_url"].rstrip("/"),
        ),
        ...
        (
            ...
            200,
            lambda data, payload: (
                data["redirect_url"].rstrip("/") == payload["redirect_url"].rstrip("/")
                and
                # Note: API may ignore color updates and keep original colors
                "fill_color" in data
                and "back_color" in data
            ),
        ),
