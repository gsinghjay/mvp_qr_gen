...
def validate_qr_code_data(data: dict[str, Any], expected: dict[str, Any] | None = None) -> bool:
    """
    ...
    if "scan_count" in data:
        validate_scan_statistics(
            {"scan_count": data["scan_count"], "last_scan_at": data.get("last_scan_at")}
        )
...
def validate_qr_code_list_response(
    response_data: dict[str, Any], expected_count: int | None = None
) -> bool:
    """
    ...
    if expected_count is not None:
        assert (
            len(response_data["items"]) == expected_count
...
def assert_successful_response(
    response, expected_status: int = status.HTTP_200_OK, expected_data: dict[str, Any] | None = None
) -> dict[str, Any]:
    """
...
def assert_validation_error(
    response, expected_status: int = status.HTTP_422_UNPROCESSABLE_ENTITY, field: str | None = None
) -> dict[str, Any]:
    """
...
def parameterize_test_cases(
    base_data: dict[str, Any], test_cases: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    """
