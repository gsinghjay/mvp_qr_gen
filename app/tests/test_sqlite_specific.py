...
def test_concurrent_reads(test_db, seeded_db, test_session_factory):
    ...
    assert all(
        count == actual_count for count in results
    ), f"Expected all counts to be {actual_count}, got {results}"
        ...
        def waiting_thread():
            ...
            try:
                ...
                try:
                    ...
                    if wait_time < 2.5:
                        error_queue.put(
                            f"Wait time was only {wait_time:.4f} seconds (expected >= 2.5)"
                        )
                    else:
...
def test_database_checkpoint(test_engine):
    ...
    metadata = MetaData()
    test_table = Table(
        test_table_name, metadata, Column("id", String, primary_key=True), Column("value", Integer)
    )
    ...
    try:
        ...
        with test_engine.connect() as conn:
            ...
            for i in range(1000):  # Increase number of rows for more reliable testing
                conn.execute(test_table.insert().values(id=str(uuid.uuid4()), value=i))
        ...
        with test_engine.connect() as conn:
            ...
            for i in range(500):
                conn.execute(test_table.insert().values(id=f"batch2_{uuid.uuid4()}", value=i * 10))
            conn.commit()
        ...
        assert (
            final_size < mid_size * 0.5 or final_size == 0 or not wal_file.exists()
    ...
    try:
        ...
        with test_engine.connect() as conn:
            ...
            try:
                ...
                assert isinstance(
                    utcnow_result, str
                ), f"Expected string but got {type(utcnow_result)}"

                # Check if it has the expected format with Z suffix for UTC
                assert utcnow_result.endswith(
                    "Z"
                ), f"Expected UTC time with Z suffix, got {utcnow_result}"
            ...
            try:
                ...
                parse_result = conn.execute(
                    text("SELECT parse_datetime(:dt_str)"), {"dt_str": test_dt_str}
                ).scalar()
