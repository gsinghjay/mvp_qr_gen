...
from app.tests.helpers import assert_http_exception
...
def test_route_mapping_consistency():
    ...
    assert any(path == "/api/v1/qr/static" or path.startswith("/api/v1/qr/static/") for path in api_routes)
    assert any(path == "/api/v1/qr/dynamic" or path.startswith("/api/v1/qr/dynamic/") for path in api_routes)
