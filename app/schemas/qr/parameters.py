...
class QRListParameters(BaseModel):
    ...
    qr_type: Optional[QRType] = Field(
        default=None, 
        description="Filter by QR code type (static or dynamic)"
    )
...
class StaticQRCreateParameters(QRCreateParameters):
    ...
    redirect_url: Optional[HttpUrl] = Field(
        default=None,
        description="Not allowed for static QR codes"
    )
    
    @field_validator("redirect_url")
    @classmethod
    def validate_no_redirect_url(cls, v: Optional[HttpUrl]) -> None:
...
class DynamicQRCreateParameters(QRCreateParameters):
    ...
    redirect_url: HttpUrl = Field(
        ..., 
        description="URL to redirect to when the QR code is scanned"
    )
