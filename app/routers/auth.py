...
async def callback(
    ...
    if error:
        logger.error(
            "Authentication error", extra={"error": error, "error_description": error_description}
        )
        return HTMLResponse(
    ...
    try:
        ...
        return HTMLResponse(
            f"<h1>Authentication Error</h1><p>{e.detail}</p>", status_code=e.status_code
        )
    ...
    logger.info(
        "User authenticated", extra={"user_id": claims.get("oid", ""), "next_url": next_url}
    )
