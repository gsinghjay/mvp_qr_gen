...
def generate_qr_image(
    ...
    """
    try:
        # Determine error correction level - use high when including a logo
        ...
        # Create QR code with high error correction if using logo
        qr = segno.make(content, error=error)
        
        # For SVG output, handle differently since it's vector-based
        if image_format.lower() == "svg":
            output = io.BytesIO()
            # SVG doesn't support logo embedding directly - we would need to add it manually
            # For simplicity, we'll just create a standard SVG QR code
            qr.save(output, kind="svg", scale=10, dark=fill_color, light=back_color, border=border)
